...
function detail(deptId){
		window.location.href="<%=request.getContextPath() %>/DeptInfo/detail?deptId=" + deptId ;
	}
	function edit(deptId){
		window.location.href="<%=request.getContextPath() %>/DeptInfo/toedit?deptId=" + deptId ;
	}
	function del(deptId){
		if(window.confirm("是否确认删除?")){
			window.location.href="<%=request.getContextPath() %>/DeptInfo/delete?deptId=" + deptId ;
		}
	}
	function add(){
		window.location.href="<%=request.getContextPath() %>/DeptInfo/add";
	}
	//test();
</script>
  <body class="main_body">
<form name='page_form' id="page_form" action='<%=request.getContextPath() %>/DeptInfo/queryForPageList' method='post'>
    ...
    <td align='center' background='<%=request.getContextPath() %>/static/images/wbg.gif' bgcolor="#EEF4EA">
      <table width="100%" height="32" border='0' cellpadding='0' cellspacing='0'>
        <tr height="30">
	        ...
	        <td width='30%' align="left"><input type='text' name='deptName' value='${item.deptName}'/></td>
...
<table width='98%'  border='0' align="center" cellpadding='1' cellspacing='1' bgcolor='#cfcfcf' style="margin-top:8px;width: 980px">
	...
	<tr align='center' bgcolor="#FFFFFF" height="26" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';">
		...
		<td>${item.deptName}</td>
		<td><c:forEach var="staff" items="${item.staff}" varStatus="index">
			${staff.staffName }|
		</c:forEach></td>
		<td>${item.createTime }</td>
		<td><a href="#" onClick="detail(${item.deptId});">查看</a> | <a href="#" onClick="edit(${item.deptId});">修改</a> | <a href="#" onClick="del(${item.deptId})">删除</a></td>
	</tr>
	</c:forEach>
...
<table width="94%" border="0" cellspacing="0" cellpadding="0">
										<tr>
											<td width="60%" height="24"><BUTTON style="height:21px; font-size:12px" onClick="javascript:if (confirm('导出部门基本信息表？')) location.href='<%=request.getContextPath()%>/DeptInfo/daoChu';else return;">导出报表</BUTTON>
											</td></tr></table>
  </body>
</html>
