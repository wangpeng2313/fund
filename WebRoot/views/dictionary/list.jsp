...
<script>
	function detail(id){
		window.location.href="<%=request.getContextPath() %>/dictionary/detail?id=" + id ;
	}
	function edit(id){
		window.location.href="<%=request.getContextPath() %>/dictionary/edit?id=" + id ;
	}
	function del(id,delType){
			if(window.confirm("是否确认删除?")){
				window.location.href="<%=request.getContextPath() %>/dictionary/delete?id=" + id ;
			}
	}
	function add(){
		window.location.href="<%=request.getContextPath() %>/dictionary/add";
	}
	//test();
</script>
<body class="main_body">
<form name='page_form' id="page_form" action='<%=request.getContextPath() %>/dictionary/queryForPageList' method='post'>
  ...
  <tr bgcolor='#EEF4EA'>
    <td align='center' background='<%=request.getContextPath() %>/static/images/wbg.gif' bgcolor="#EEF4EA">
      <table width="100%" height="32" border='0' cellpadding='0' cellspacing='0'>
        <tr height="30">
	        <td width="20%" align="right">&nbsp;类型编码： </td>
	        <td width="30%" align="left"><input type="text" name="typeCode" value="${param.typeCode }"/></td>
...
<table width='98%'  border='0' align="center" cellpadding='1' cellspacing='1' bgcolor='#cfcfcf' style="margin-top:8px;width: 980px">
	...
	<tr align='center' bgcolor="#FFFFFF" height="26" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';">
		...
		<td>${item.parentId }</td>
		<td>${item.sort }</td>
		<td><a href="#" onClick="detail(${item.dictionaryId});">查看</a> | <a href="#" onClick="edit(${item.dictionaryId});">修改</a> | <a href="#" onClick="del(${item.dictionaryId})">删除</a></td>
	</tr>
	</c:forEach>
